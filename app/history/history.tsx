...
const ChatHistory: FC<ChatHistoryProps> = ({ userId }) => {
  ...
  const ChatMessage: FC<{ message: Message }> = ({ message }) => {
    return (
      <div className={`flex ${message.role === "assistant" ? "justify-start" : "justify-end"} mb-2`}>
        <div
          className={`flex items-center ${message.role === "assistant" ? "bg-gray-200 text-gray-900" : "bg-blue-500 text-white"} rounded-2xl px-4 py-2 max-w-[67%] shadow-md`}
          style={{ overflowWrap: "anywhere" }}
        >
          <Markdown text={message.content} />
        </div>
  ...
  return (
    <div className="flex flex-col space-y-4 p-6 max-w-3xl mx-auto bg-white rounded-lg shadow-lg h-[80vh] overflow-auto border border-gray-200">
      <h3 className="text-3xl font-extrabold mb-4 text-center text-gray-800">Chat History</h3>
      {messages.length === 0 ? (
